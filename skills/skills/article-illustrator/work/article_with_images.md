...
原文链接: https://mp.weixin.qq.com/s/v2ZSlprwT9IGD_S98listg
...
![渐进式加载机制](imgs/002-progressive-loading.png)
...
至于和 MCP 的区别？MCP 是用来统一工具调用的协议，Skill 可以指挥 Agent 去调用 MCP。
...
![五步工作流程](imgs/003-five-step-workflow.png)
...
![Skill 文件结构](imgs/004-skill-structure.png)
...
为什么要单独写这个模板？因为图像生成的约束条件是通用的（比如比例、手绘风格、不用写实元素），而具体内容是变化的。把通用部分抽出来，既减少重复，也方便统一调整。

![提示词组装过程](imgs/005-prompt-assembly.png)
...
![端到端完整流程](imgs/006-end-to-end-flow.png)
...
核心就是把你脑子里那些"做这件事的正确方法"写下来，交给 Agent 执行。
